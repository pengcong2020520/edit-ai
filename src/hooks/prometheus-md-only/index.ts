...
const TASK_TOOLS = ["chief_task", "task", "call_omo_agent"]
  ...
  return {
    "tool.execute.before": async (
      ...
      if (!isAllowedFile(filePath)) {
        log(`[${HOOK_NAME}] Blocked: Prometheus can only write to .chief/*.md`, {
          sessionID: input.sessionID,
          tool: toolName,
          filePath,
          agent: agentName,
        ...
        throw new Error(
          `[${HOOK_NAME}] Prometheus (Planner) can only write/edit .md files inside .chief/ directory. ` +
          `Attempted to modify: ${filePath}. ` +
          `Prometheus is a READ-ONLY planner. Use /start-work to execute the plan.`
        )
      }

      log(`[${HOOK_NAME}] Allowed: .chief/*.md write permitted`, {
        sessionID: input.sessionID,
        tool: toolName,
        filePath,
        agent: agentName,
